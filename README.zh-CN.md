...
项目索引通过 `dialogue_mode: fast|deep` 记录对话方式：

- `deep` 是默认的深度共创模式，适合新产品、体验方向、架构、安全、支付或较大切片变更。AI 会在相关验收门禁前一次提出一个或多个相互独立的高价值问题，说明每题为何重要，并给出推荐默认项及其主要取舍；依赖前一题答案的问题留到下一轮。
- `fast` 是快速推进模式。AI 只询问无法从证据回答、且会阻塞或实质改变结果的决策；说“切换到快速推进模式”即可切换。

The First 会在首次响应中声明当前使用深度共创模式，并说明如何切换到快速推进模式。深度共创通常包含 3～7 个问题；实际问题更少时提前结束。AI 汇总已决定、已延后和仍有风险的事项后恢复正常执行。对话深度不会新增验收门禁，也不会授权写入、安装、推送或部署。
...
`guard-artifact-scope` 是跨阶段守卫。只有当同一产物混合面向读者的产品内容与内部约束，且仍存在真实的范围冲突时，AI 才会隐式调用；普通文档修改继续由对应阶段 skill 处理，用户仍可显式调用，守卫本身不会增加阶段或验收门禁。
