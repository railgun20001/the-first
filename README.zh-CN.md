...
- `fast` 是默认的快速推进模式。AI 只询问无法从证据回答、且会阻塞或实质改变结果的决策。
- `deep` 是显式开启的深度共创模式，适合新产品、体验方向、架构、安全、支付或较大切片变更。AI 会在相关验收门禁前逐个询问少量高价值问题，说明问题为何重要，并给出推荐默认项及其主要取舍。

深度共创通常包含 3～7 个问题；实际问题更少时提前结束。AI 汇总已决定、已延后和仍有风险的事项后恢复正常执行。对话深度不会新增验收门禁，也不会授权写入、安装、推送或部署。
