...
5. 面向用户的产品默认前端/UI 或游戏客户端先行或并行，让人能在后端大规模完成前操作和修改，而非事后交付 demo。
...
每个功能切片必须有可观察结果、范围、验收标准、自动化验证和用户自测流程。开发前，AI 会把当前切片的稳定需求或验收引用映射到预期实现面和验证方式；开发后逐项回填实际代码或运行证据。任一范围内条目缺少证据时，切片保持未完成。用户验收前不把切片标记为完成；用户提出修改时，AI 会记录底层意图、长期规则、受影响范围和回归保护。
