...
5. 面向用户的产品默认前端/UI 或游戏客户端先行或并行，让人能在后端大规模完成前操作和修改，而非事后交付 demo；完整玩法切片在视觉/UI 验收通过前不得标记完成。
6. 任何软件安装、外部写入、代码推送或生产部署都需要对应授权。
7. 开发前将当前切片的需求和验收项映射到实现面与验证方式；缺少逐条证据时不得完成。
8. 用户修改先被提炼为可复用规则，再进入代码、验收和回归保护。
9. 功能通过验证和用户验收后，按功能创建独立 Git 提交。
...
面向用户的产品默认让前端/UI 或游戏客户端先行或并行于后端大规模开发。使用静态或契约形态的 mock 数据覆盖已确认的主流程和关键状态，让用户能实际操作、验收并提出修改；在极端的前端先行路径中，完整且已验收的客户端流程可以反推后续 API、服务与持久化工作。
...
每个功能切片必须有可观察结果、范围、验收标准、自动化验证和用户自测流程。开发前，AI 会把当前切片的稳定需求或验收引用映射到预期实现面和验证方式；开发后逐项回填实际代码或运行证据。任一范围内条目缺少证据时，切片保持未完成。完整玩法切片还必须记录对可操作客户端的明确视觉/UI 人工验收；截图、静态检查和后端测试均不能替代。用户验收前不把切片标记为完成；用户提出修改时，AI 会记录底层意图、长期规则、受影响范围和回归保护。
