...
2. Confirm the requirement phase has sufficient evidence before producing implementation-ready experience decisions. For discussion-only exploration, keep proposals explicitly unconfirmed.
...
Provide a short optional review path for the key flow and uncertain states. Collect feedback about outcomes and interaction, not merely visual preference.
...
The prototype is evidence, not final authority. User-stated outcomes and current sources of truth override it. Production implementation should preserve evidence-backed behavior without being required to copy prototype code or pixels.
...
When the documented experience and its required checks provide sufficient evidence, mark the phase complete, record it through `$track-project-progress`, and route to `$design-technical-solution`. Ask the user only to resolve an evidence gap or material product conflict.
