...
In Codex, prefer the available built-in question UI over asking the same questions in prose. Use `request_user_input_async` when exposed and permitted, batching independent questions in its `questions` array with concise suggested options and free-text support. Continue independent authorized work while its answer is pending; keep dependent work pending until the answer arrives. If only `request_user_input` is available, use it only when the current mode and tool instructions permit the question, respecting its question limit. Tool names may be namespaced: use the callable tool exposed by the host. Do not switch modes just to obtain a question UI, treat a preselected option or elapsed time as an answer, or use a clarification tool for permissions it does not support. If no suitable tool is available, ask the same numbered batch in ordinary conversation. This tool preference applies in both dialogue modes and all phases that need clarification.
